...
export default function Service({ data }) {
  ...
  return (
    <Section id="service" title="Professional Service & Activities" icon={<FaUserTie />}>
      ...
      {service.length > 0 && (
        <div className="mb-12">
          <h3 className="text-lg font-bold mb-4" style={{ color: 'var(--color-text)' }}>Reviewer & Committee Roles</h3>
          <div className="grid md:grid-cols-2 gap-3">
            {service.map((s, i) => (
              <div
                key={i}
                className="flex items-start gap-3 rounded-lg p-4 border transition-all hover:shadow-md"
                style={{ backgroundColor: 'var(--color-card)', borderColor: 'var(--color-border)' }}
              >
                <span className="mt-0.5 text-sm" style={{ color: 'var(--color-accent)' }}>✦</span>
                <div>
                  <span className="text-xs font-bold px-2 py-0.5 rounded mr-3 inline-block mb-1"
                  ...
                  <p className="text-xs mt-1" style={{ color: 'var(--color-text-secondary)' }}>{s.details}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mb-12">
          ...
          <div className="space-y-3">
            {workshops.map((w, i) => (
              <div
                key={i}
                className="rounded-lg p-4 border-l-3 transition-all hover:shadow-md"
                style={{
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div className="flex-1">
                    ...
                    <p className="text-xs mt-1" style={{ color: 'var(--color-text-secondary)' }}>{w.organizer}</p>
                  </div>
                  <div className="flex gap-2 flex-shrink-0">
                    ...
                    <span className="px-2 py-0.5 rounded text-xs" style={{ backgroundColor: 'var(--color-bg-alt)', color: 'var(--color-text-secondary)' }}>
                      {w.period}
                    </span>
                  </div>
                </div>
      ...
      {admin.length > 0 && (
        <div>
          <h3 className="text-lg font-bold mb-4" style={{ color: 'var(--color-text)' }}>Administrative Activities</h3>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {admin.map((a, i) => (
              <div
                ...
                style={{ backgroundColor: 'var(--color-card)', borderColor: 'var(--color-border)' }}
              >
                <h4 className="font-bold text-sm mb-1" style={{ color: 'var(--color-text)' }}>{a.role}</h4>
                <p className="text-xs font-medium mb-2" style={{ color: 'var(--color-accent)' }}>{a.institution}</p>
                <p className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>{a.description}</p>
              </div>
            ))}
          </div>
        </div>
