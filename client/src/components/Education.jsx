    ...
    <Section id="education" title="Education" icon={<FaGraduationCap />} alt>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr style={{ backgroundColor: 'var(--color-primary)', color: 'white' }}>
              <th className="px-6 py-4 text-left text-sm font-semibold rounded-tl-lg">Program</th>
              ...
              <th className="px-6 py-4 text-center text-sm font-semibold rounded-tr-lg">Year</th>
            </tr>
          </thead>
          <tbody>
            {data.education.map((edu, i) => (
              ...
              >
                <td className="px-6 py-4 font-semibold text-sm" style={{ color: 'var(--color-text)' }}>
                  {edu.program}
                </td>
      ...
      <div className="md:hidden space-y-3">
        {data.education.map((edu, i) => (
          ...
          >
            ...
            <p className="text-xs mb-2" style={{ color: 'var(--color-text-secondary)' }}>{edu.institution}</p>
            <span
      ...
      {data.theses?.length > 0 && (
        <div className="mt-16 md:mt-24">
          <h3 className="text-lg md:text-2xl font-bold mb-4 md:mb-6" style={{ color: 'var(--color-text)' }}>
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
            {data.theses.map((thesis, i) => (
              <div
                key={i}
                className="rounded-xl p-5 md:p-6 border transition-all duration-300 hover:-translate-y-1 hover:shadow-lg"
                style={{ backgroundColor: 'var(--color-card)', borderColor: 'var(--color-border)' }}
              >
                ...
                <h4 className="font-semibold text-sm md:text-base mb-2" style={{ color: 'var(--color-text)' }}>{thesis.title}</h4>
